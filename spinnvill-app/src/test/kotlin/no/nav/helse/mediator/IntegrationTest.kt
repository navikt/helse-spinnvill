...
internal class IntegrationTest {
    ...
    @Test
    fun `be om sammenligningsgrunnlag når det ikke er gjort noen avviksvurdering tidligere`() =
        medTestContext(1 jan 2018) {
    ...
    @Test
    fun `ignorerer historikk og foretar dermed ny avviksvurdering dersom gjeldende avviksvurderingsgrunnlag er av type Infotrygd`() =
        medTestContext(1 jan 2018) {
            mottaAvviksvurderingBehov(600_000.0)
    ...
    fun `gjør ikke ny avviksvurdering når vi har avviksvurdering fra før og beregningsgrunnlag bare er litt forskjellig`() =
        medTestContext(1 jan 2018) {
            ...
            val løsningNode2 =
                testRapid.inspektør
                    .behov("Avviksvurdering")[1]
                    .path("@løsning")
                    .path("Avviksvurdering")
            val løsningNode3 =
                testRapid.inspektør
                    .behov("Avviksvurdering")[2]
                    .path("@løsning")
                    .path("Avviksvurdering")
    ...
    private fun TestContext.mottaSammenligningsgrunnlag(innrapportertÅrsinntektÅrlig: Double) {
        ...
        val gruppertPåÅrMåned =
            this
                .nyttSammenligningsgrunnlag(innrapportertÅrsinntektÅrlig)
                .inntekter
                .flatMap { (key, values) ->
        ...
        val inntekterForSammenligningsgrunnlag =
            objectMapper.convertValue(
                mapOf(
                    "InntekterForSammenligningsgrunnlag" to
                        gruppertPåÅrMåned.map {
                            mapOf(
                                "årMåned" to it.key,
                                "inntektsliste" to
                                    it.value.map { (organisasjonsnummer, månedligInntekt) ->
                                        mapOf(
    ...
    private fun Beregningsgrunnlag.toJson(): String =
        objectMapper.writeValueAsString(
            this.omregnedeÅrsinntekter.map {
                mapOf(
